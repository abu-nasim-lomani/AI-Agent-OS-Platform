...
BEGIN
  FOREACH t IN ARRAY ARRAY[
    ...
    IF t = 'organizations' THEN
      EXECUTE format(
        'CREATE POLICY tenant_isolation ON %I USING (id = current_setting(''app.current_org_id'')::uuid)', t);
    ELSE
      EXECUTE format(
        'CREATE POLICY tenant_isolation ON %I USING (org_id = current_setting(''app.current_org_id'')::uuid)', t);
    END IF;
